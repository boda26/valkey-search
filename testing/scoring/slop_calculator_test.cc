...
// Case 6: flat OR root unwrapped -> distance across branches (matches the
...
// Case 8: OR groups stay in query order (an intentional divergence from the
// reference implementation).
...
// Case 10: gap 0 -> zero-sum fallback to anchor count.
// red red, red at {0,6} : {0,6},{0,6} -> MinGap 0 -> 2 anchors - 1 -> 1.
...
// Case 10b: with three anchors the zero-sum fallback is no longer 1.
// red red red, red at {0,6} : every MinGap is 0 -> 3 anchors - 1 -> 2.
TEST(SlopCalculatorTest, ZeroSumFallsBackToAnchorCount) {
  SlopCalculator calc;
  Term(calc, {0, 6});  // red
  Term(calc, {0, 6});  // red
  Term(calc, {0, 6});  // red
...
// Case 10c: an index created NOOFFSETS stores every token at position 0, so
// every gap is 0 and the slop degrades to the anchor count: 5 terms -> 4.
TEST(SlopCalculatorTest, NoOffsetsDegradesToAnchorCount) {
  SlopCalculator calc;
  for (int i = 0; i < 5; ++i) {
    Term(calc, {0});
  }
  EXPECT_EQ(calc.Finalize(), 4u);
}
...
// One calculator serves consecutive documents: Reset must drop the previous
// walk entirely, including an abandoned open group.
TEST(SlopCalculatorTest, ResetReusesCalculatorAcrossDocuments) {
  SlopCalculator calc;
  Term(calc, {0});  // apple
  Term(calc, {3});  // banana
  EXPECT_EQ(calc.Finalize(), 3u);
  ...
  // A walk abandoned mid-group (the scoring path drops documents that fail to
  // match) must not leak its open group into the next document.
  calc.Reset();
  calc.EnterGroup();
  Term(calc, {5});
  calc.Reset();
